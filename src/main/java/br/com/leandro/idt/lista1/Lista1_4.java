...
public class Lista1_4 {
    public static void main(String[] args) {
        ...
        if (idade < 16 && idade >= 0) { System.out.printf("\nCom %d anos, você não possui idade para votar.", idade); }
        else if (idade >= 16 && idade < 18 || idade >= 65) { System.out.printf("\nCom %d anos, você é um eleitor facultativo.", idade); }
        else if (idade >= 18 && idade < 65) { System.out.printf("\nCom %d anos, você é um eleitor obrigatório.", idade); }
        else { System.out.println("\nIdade inválida, tente novamente."); }

        System.out.print("\n\nPressione ENTER para encerrar...");
        s.nextLine(); s.close(); LimparTela();
